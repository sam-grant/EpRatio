...
void draw(TH1D *hist1, TH1D *hist2, TFile *output, string name, string title) {
  ...
  // Collect stats of second histogram
...
int main() {
  ...
  // Set input
  string inputFnameEp = "EpParameters_Q.root";
  string inputFnameLaser = "LaserParameters.root";
  TH1D *Ep, *laser;
  ...
  cout<<"\nReading... "<<inputFnameEp<<", "<<inputEp<<endl;
  cout<<"\nReading... "<<inputFnameLaser<<", "<<inputLaser<<"\n"<<endl;
  
  // Set output
  string outputFname = "Pull.root";
  ...
  for (int ihist(0); ihist < 4; ihist++) {
	...
	cout<<"**Some kind of error, please fix**"<<endl;
	return 0;
      }
  ...
  cout<<"\nFilling complete, drawing"<<endl;
  
  pullHistAmp->GetYaxis()->SetRangeUser(0,7);
  draw(pullHistTau,pullHistAmp,output,"PullOverlay1D.png","Stn 12 & 18;Pull [#sigma];Entries");
  ...
  cout<<"\n"<<outputFname<<", "<<output<<", created, exiting..."<<endl;

...
#please, no sourcing!
[[ $0 != $BASH_SOURCE ]] && echo "Please do not source the script, run it via $BASH_SOURCE instead." && return
 
#make sure that the helper scripts exists and are executable
[ ! -r $PWD/submitGM2Data.sh ] && echo "Unable to access submitGM2Data.sh at $PWD, please run from release area!" && exit 1
[ ! -r $PWD/GM2DataParserUtils.py ] && echo "Unable to access GM2DataParserUtils.py at $PWD, please run from your release area!" && exit 1
[ ! -r $PWD/hadd_script.sh ] && echo "Unable to access hadd_script.sh at $PWD, please run from release area!" && exit 1
...
TMPDIR=`mktemp -d` || ( mkdir -p /tmp/$USER && TMPDIR=/tmp/$USER )
if [ ! -d $TMPDIR -o ! -w $TMPDIR ]; then
     echo -e "\n!!!Unable to create temporary directory at '$TMPDIR' !!!\n"
     exit 1
fi
...
#Same as in GM2DataParserUtils.py
OS="SL6"    #default is SL6 OS
...
do
   ...
   if [[ $key == "subgroup" ]] ; then
      [[ ! -z $value ]] && [[ $value != "None" ]] && SUBGROUP_OPT="--subgroup $value"
   fi
   ...
   if [[ $key == "input-filelist" ]] ; then
      INPUT_FILELIST="$value"
   fi

   if [[ $key == "input-dir" ]] ; then
      INPUT_DIR="$value"
   fi

   if [[ $key == "input-ext" ]] ; then
      INPUT_EXT="$value"
   fi
...
if [[ ${INPUT_FILE} != "None" ]] || [[ ${INPUT_FILELIST} != "None" ]] || [[ ${INPUT_DIR} != "None" ]]; then

   USEDATASET=0
   if [ $USESAMD -eq 1 ]; then
   	echo -e "\n!!!Cannot use SAM dataset and input-file/input-dir/input-filelist options at the same time!!!\n"
   	exit 1
   fi

   if [ ${IFDH_ART} -eq 1 -a $HADD -eq 0 ]; then
   	echo -e "\n!!!IFDH_ART interface not supported for input-file/input-dir/input-filelist options, use --noifdh_art instead!!!\n"
   	exit 1
   fi
...
if [[ ${INPUT_FILE} != "None" ]]; then
        if [[ ${INPUT_FILELIST} != "None" ]]; then
   	      ...
   	      exit 1
        elif [[ ${INPUT_DIR} != "None" ]]; then
   	      echo -e "\n!!!Cannot use input-file and input-dir at the same time!!!\n"
   	      exit 1
        else
              echo -e "\nInput file: ${INPUT_FILE}" | tee -a $LOG
        fi
fi
...
if [[ ${INPUT_DIR} != "None" ]]; then
      ...
      if [[ ! -d "${INPUT_DIR}" ]] || [[ ! -r "${INPUT_DIR}" ]]; then
            echo -e "\n!!!Input directory '${INPUT_DIR}' cannot be accessed!!!\n" 
            exit 1
      else
            echo -e "\nInput directory: ${INPUT_DIR}" | tee -a $LOG
      fi
      [[ ${INPUT_FILELIST} != "None" ]] && ILIST="${INPUT_FILELIST}"
      ([[ ! -z $ILIST ]] && cat "$ILIST"; find ${INPUT_DIR} -type f -name "*.${INPUT_EXT}") | sort | uniq > "$TMPDIR/$(basename ${INPUT_DIR}).list"
      if [ $? -eq 0 ]; then
         INPUT_FILELIST="$TMPDIR/$(basename ${INPUT_DIR}).list"
         if [[ -z $ILIST ]]; then
               echo -e "\nInput filelist generated at '${INPUT_FILELIST}' from input directory" | tee -a $LOG
         else
               echo -e "\nInput filelist generated at '${INPUT_FILELIST}' from input directory and filelist" | tee -a $LOG
         fi
      else
         echo -e "\n!!!Unable to create file list '$TMPDIR/$(basename ${INPUT_DIR}).list' from input directory '${INPUT_DIR}' !!!\n"
         exit 1 
      fi
fi
...
if [ $HADD -eq 1 ]; then
     if [[ ${INPUT_FILE} != "None" ]]; then
   	echo -e "\n!!!No need to run 'hadd' with a single input file!!!\n"
   	exit 1
     fi
...
if [[ $HADD -eq 0 ]] && [[ ${MAINFCLNAME} == "None" ]] ; then
...
#make sure CAMPAIGN is set properly if SAMDATASET or REQUESTID is defined
      ...
      [[ $HADD -eq 1 ]]           && CAMPAIGN="hadd"

#make sure maxconcurrent is set to be 10000 for production jobs
      MAXCONCURRENT_OPT="--maxConcurrent 10000"

#make sure job subgroup is set to be 'prod' by default for production jobs
      [[ $DEBUG -eq 0 ]] && [[ -z ${SUBGROUP_OPT} ]] && SUBGROUP_OPT="--subgroup prod"
#make sure simulation production jobs are assigned to 'sim' subgroup
      [[ $DEBUG -eq 0 ]] && [[ ${DATA_TYPE} == "mc" ]] && [[ -z ${SUBGROUP_OPT} ]] && SUBGROUP_OPT="--subgroup sim"
fi


if [ $GRID -eq 0 ]; then 
     [ ! -w $PWD ] && echo "Unable to write to current directory at $PWD when running jobs locally, please fix it!" && exit 1 
     if [[ ! -z ${mrb_command} ]]; then 
...
if [ $USEDATASET -eq 1 ] ; then
   ...
   if [ $HADD -eq 0 ] && [[ ! -z $SAM_FILE_LIMIT ]] && [[ $SAM_FILE_LIMIT -gt 0 ]] && [[ $SAM_FILE_LIMIT -lt $SAMDATASET_MAX_FILES ]] ; then
      ...
      echo "***It is OK to do this if you are running a test or you have a hard limit on the number of input files per job," | tee -a $LOG
      echo "***although this might hurt the overall job efficiency (check details at $HELP)." | tee -a $LOG
      echo "***The recommended way to run over all files from ${SAMDATASET} dataset is to NOT set 'sam-max-files' at all." | tee -a $LOG
      echo "***This will allow all ${SAMDATASET_MAX_FILES} files to be (randomly and more or less evenly) consumed by $NJOBS jobs and produce one output file per job." | tee -a $LOG
      echo "***If you require one output file per input file instead, please also activate --noifdh_art and --onebyone options." | tee -a $LOG 
      echo -e "***If you wish to change the configuration and re-submit, Ctrl+C to exit now!\n" | tee -a $LOG
      sleep 12
   fi
...
NINPUT=0
NPERJOB=-1
TMPLIST=$TMPDIR/$(basename ${INPUT_FILELIST}).$SCHEMAS
...
for MAINFCLNAME in "${FCLNAMES[@]}"; do
    ( [[ -z $MAINFCLNAME ]] || [[ $MAINFCLNAME == "None" ]] ) && continue
...
if [ ${IFDH_ART} -eq 1 ]; then
      echo "IFDH_ART interface activated" | tee -a $LOG
      ...
      echo "IFDH_ART interface disabled"  | tee -a $LOG
fi
...
if [[ "${OUTPUT_DIR}" == "None" ]] ; then
   if [ $HADD -eq 0 ]; then
        ...
        OUTPUT_DIR=/pnfs/GM2/scratch/users/${USER}/${CAMPAIGN}
   fi
fi
...
if [ $USELOCAL -eq 1 ]; then
	...
	if [ $GRID -eq 1 ]; then
                ...
                if [ ! -d ${OUTPUT_PNFS_DIR} ]; then
                        echo -e "\n\tUnable to make directory at ${OUTPUT_PNFS_DIR}, use $TMPDIR instead ..." | tee -a $LOG
                ...
                if [[ -e $TARFILE ]] && [[ $NEWLOCAL -eq 0 ]]; then
                        ...
                        if rm -f $TARFILE; then
                           if tar cfz $TMPDIR/$TARNAME -C / ${MRB_INSTALL/\//}; then
                              [ -d ${OUTPUT_PNFS_DIR} ] && mv $TMPDIR/$TARNAME ${OUTPUT_PNFS_DIR}
                           else
                              echo -e "\n!!!Unable to make tar file $TMPDIR/$TARNAME, check /tmp space!!!\n" | tee -a $LOG  && exit 1
                           fi
                        else
...
#################################
if [ $HADD -eq 0 ]; then
     INPUTLIST="${opts} runNumber ${RUNNUMBER} remoteFile ${RFCLNAMELIST[0]} remoteFiles $RFCLNAME outfiles ${OUTPUT_DIR}"
else
     INPUTLIST="${opts} outfiles ${OUTPUT_DIR}"
fi
...
###################################
# Ship SL6 image to a non-SL6 node
###################################
if [ $SL6 -eq 1 ]; then
   LINES_OPT2=+SingularityImage=\"/cvmfs/singularity.opensciencegrid.org/fermilab/fnal-wn-sl6:latest\"
   if [[ $OS == "SL6" ]]; then
         OS_OPT="--append_condor_requirements='(TARGET.HAS_SINGULARITY=?=true||TARGET.IFOS_Installed=?=\"SL6\")'"
   else
         OS_OPT="--append_condor_requirements='(TARGET.HAS_SINGULARITY=?=true)'"
   fi
fi

###################################
# Ship SL7 image to a non-SL7 node
###################################
if [ $SL7 -eq 1 ]; then
   LINES_OPT2=+SingularityImage=\"/cvmfs/singularity.opensciencegrid.org/fermilab/fnal-wn-sl7:latest\"
   if [[ $OS == "SL7" ]]; then
         OS_OPT="-append_condor_requirements='(TARGET.HAS_SINGULARITY=?=true||TARGET.IFOS_Installed=?=\"SL7\")'"
   else
         OS_OPT="--append_condor_requirements='(TARGET.HAS_SINGULARITY=?=true)'"
   fi
fi
...
##################################
if [ $HADD -eq 0 ]; then
     SCRIPT_OPT="file://$PWD/submitGM2Data.sh ${INPUTLIST}"
else
     SCRIPT_OPT="file://$PWD/hadd_script.sh ${INPUTLIST}"
fi
...
if [ $GRID -eq 1 ]; then
   ...
   TIER=${DATA_TIER}
   echo -e "\nsource ./$(basename $(echo ${SCRIPT_OPT} | cut -d' ' -f1)) $INPUTLIST ..." | tee -a $LOG
   [ $TEST -eq 0 ] && source ./$(basename $(echo ${SCRIPT_OPT} | cut -d' ' -f1)) $INPUTLIST
fi

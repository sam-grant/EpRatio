...
void Plotter::InitHistos() {
  ...
  for (int stn(13); stn < 20 ; stn = stn + 6) {
    plot2D("St"+std::to_string(stn)+"_E_vs_t",cycles,0,4200*cycles,120,0,4000,"In fill time [ns]","Cluster energy [MeV]");
    plot2D("St"+std::to_string(stn)+"_p_vs_t",cycles,0,4200*cycles,120,0,4000,"In fill time [ns]","Track momentum [MeV]");
...
void Plotter::Run() {
  ...
  while( NextallmuonsEvent() ) {
    ...
    for(int i=0; i<am->nmatches; i++) {
      ...
      Fill2D("St"+std::to_string(caloSt)+"_E_vs_t",t,E);
      Fill2D("St"+std::to_string(caloSt)+"_p_vs_t",t,p);

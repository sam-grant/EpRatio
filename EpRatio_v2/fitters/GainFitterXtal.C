...
void GainSagFit(auto *hist, double binning, int start, double end) {
...
void FancyDraw(auto *hist, string title, TFile *output, int binning, int cycles) {
...
int main() {
  ...
  for (int stn(12); stn < 19; stn = stn + 6) {
    ...
    for (int xtal(0); xtal < 54; xtal++) {
      ...
      auto *hist = (TGraphErrors*)input->Get(h.c_str());
...
/*        if (N < 50000) continue;*/
      ...
      cout<<xtal<<","<<chiSqrNDF<<","<<tau<<","<<tau_err<<","<<A<<","<<A_err<<endl;

from ROOT import TH1, TH2, TGraphErrors, TCanvas, TLegend, gStyle, TPad, TGaxis, TLine, gPad, TPaveText
...
def FancyDraw1DText(hist, title, fname): 

	c = TCanvas("c","c",800,600)

	hist.SetStats(0)

	txt = TPaveText(0.59,0.69,0.89,0.89,"NDC")
	txt.AddText("#splitline{Mean = "+str('%s' % float('%.1g' % hist.GetMean()))+"#pm"+str('%s' % float('%.1g' % hist.GetMeanError()))+"#sigma}{Width = "+str('%s' % float('%.3g' % hist.GetStdDev()))+"#sigma}")
	txt.SetFillColor(0)
	txt.SetTextFont(44)
	txt.SetTextSize(26)
	txt.SetBorderSize(0)
	...
	# leg.AddEntry(hist,"#splitline{"+str(hist.GetName())+"}{Mean = "+str('%s' % float('%.2g' % hist.GetMean()))+"#pm"+str('%s' % float('%.1g' % hist.GetMeanError()))+"}{Sigma = "+str('%s' % float('%.2g' % hist.GetStdDev()))+"}")
...
def FancyDraw1DYLine(hist, title, fname, xcoord): 
...
def FancyDraw1D2YLine(hist, title, fname): 
	...
	hist.Draw()
	gPad.Update()
	line1 = TLine(-0.3,gPad.GetUymin(),-0.3,gPad.GetUymax())
	line2 = TLine(0.2,gPad.GetUymin(),0.2,gPad.GetUymax())
...
def FancyDraw1DOverlay4YLine(hist1, hist2, title, fname): 

	c = TCanvas("c","c",800,600)

	hist1.SetTitle(title)

	leg = TLegend(0.69,0.69,0.89,0.89)
	#leg.SetHeader("Mean time offset")
	leg.AddEntry(hist1,"Station 12") # ,"#splitline{"+str(hist1.GetName())+"}{Mean = "+str('%s' % float('%.2g' % hist1.GetMean()))+"#pm"+str('%s' % float('%.1g' % hist1.GetMeanError()))+" ns}")
	leg.AddEntry(hist2, "Station 18") # ,"#splitline{"+str(hist2.GetName())+"}{Mean = "+str('%s' % float('%.2g' % hist2.GetMean()))+"#pm"+str('%s' % float('%.1g' % hist2.GetMeanError()))+" ns}")
	...
	line1 = TLine(-8,gPad.GetUymin(),-8,gPad.GetUymax())
	line2 = TLine(3,gPad.GetUymin(),3,gPad.GetUymax())
	line3 = TLine(-9,gPad.GetUymin(),-9,gPad.GetUymax())
	line4 = TLine(1,gPad.GetUymin(),1,gPad.GetUymax())

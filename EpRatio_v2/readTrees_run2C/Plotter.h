...
public:
  ...
  bool NextTrackAndTrackCaloEvent(){return ttc->NextEvent();}
  ...
  void plot1DTr(TString name, int nbins, float xlow, float xhigh, TString xtitle="x axis", TString ytitle="", int col=1);  
  void Fill1DTr(TString name, double val, double weight=1);
  void plot2DTr(TString name, int xnbins, float xlow, float xhigh, int ynbins, float ylow, float yhigh, TString xtitle="x axis", TString ytitle="", int col=1);
...
void Plotter::Fill1DTr(TString name, double val, double weight) {
  Fill1D( "St"+std::to_string(ttc->station)+"_"+name, val, weight);
  //Fill1D( "St"+std::to_string(tr->station)+"_"+name, val, weight);
}
...
void Plotter::Fill2DTr(TString name, double xval, double yval, double weight) {
  Fill2D( "St"+std::to_string(ttc->station)+"_"+name, xval, yval, weight);
  //Fill2D( "St"+std::to_string(tr->station)+"_"+name, xval, yval, weight);
}
...
int main(int argc, char *argv[]){
  ...
  if (argc < 2){
    ...
    fileNames.push_back("/gm2/data/g2be/Production/TrackAndTrackCaloTrees/Run1/60h/trackAndTrackCaloTrees_15921.root");//../run15921/trackAndTrackCaloTree_15921.root");
    //../test/trackAndTrackCaloTree.root");//"/gm2/data/g2be/Production/Trees/Run2/NoDQC/trackRecoTrees_24310.root");
  }

...
void Plotter::InitHistos() {
  ...
  for (int stn = 13; stn < 20 ; stn = stn + 6) {
    for (int fidXtal = 0 ; fidXtal < 3 ; fidXtal++) { 
        plot2D("St"+std::to_string(stn)+"_Ep_vs_t_early_"+std::to_string(fidXtal),50,0,4200*50,200,ymin,ymax,"t [ns]", "E/p");
    }
  }
...
void Plotter::Run() {
  ...
  while( NextallmuonsEvent() ) {
	...
	if(dt<-9 || dt>1) continue;        
      ...
      // Terrible method of scaling gain per crystal
      ...
      double scaleFactor = scale_0->GetBinContent(xTal); 
      std::cout<<xTal<<" "<<scaleFactor<<std::endl;


      Fill2D("St"+std::to_string(caloSt)+"_Ep_vs_t_early_"+std::to_string(fidXtal),t,Ep);

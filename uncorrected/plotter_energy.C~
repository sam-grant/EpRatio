...
void Plotter::InitHistos() {
  ...
  for (int stn = 13; stn < 20 ; stn = stn + 6) {
    for (int fidMain = 0 ; fidMain < 3 ; fidMain++) { 
        plot2D("St"+std::to_string(stn)+"_Ep_vs_E_"+std::to_string(fidMain),20,0,4000,200,ymin,ymax,"Energy in calo [MeV]", "E/p");
	plot2D("St"+std::to_string(stn)+"_xy_calo_"+std::to_string(fidMain),200,-150,150,200,-120,120,"Calo X [mm]", "Calo Y [mm]");
      }
  }
}
...
void Plotter::Run() {
  ...
  while( NextallmuonsEvent() ) {
	...
	if(dt<-9 || dt>1) continue;        
      ...
      int fidMain;
      if(FiducialMain(trX,trY)) {fidMain = 1;}
      else if(!FiducialMain(trX,trY)) {fidMain = 2;}
       Fill2D("St"+std::to_string(caloSt)+"_Ep_vs_E_0",E,Ep);
       Fill2D("St"+std::to_string(caloSt)+"_Ep_vs_E_"+std::to_string(fidMain),E,Ep);
     Fill2D("St"+std::to_string(caloSt)+"_xy_calo_0",trX,trY);
       Fill2D("St"+std::to_string(caloSt)+"_xy_calo_"+std::to_string(fidMain),trX,trY);

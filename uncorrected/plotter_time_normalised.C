...
void Plotter::Run() {
 ...
 TFile *input1 = TFile::Open("../makePlots/fitted_gain_by_xtal.root");  
 TFile *input2 = TFile::Open("../makePlots/fits_xtal_energy_normalised.root");
 TFile *input3 = TFile::Open("../makePlots/fits_xtal_energy.root");
 ...
 while( NextallmuonsEvent() ) {
	...
	if (xtal == shortLifeXtal[i]) {
      ...
      std::string h3 = "St"+std::to_string(caloSt)+"_fit_E_vs_xtal_0";//_"+std::to_string(xtal);
      std::string h4 = "St"+std::to_string(caloSt)+"_fit_E_vs_xtal_1";//_"+std::to_string(xtal);
      
      TH1D *energyNorm1 = (TH1D*)input2->Get(h3.c_str());
      if(energyNorm1 == 0) continue;
      TH1D *energyNorm2 = (TH1D*)input2->Get(h4.c_str());
      if(energyNorm2 == 0) continue;

      double mic = energyNorm1->GetBinContent(energyNorm1->FindFirstBinAbove(0));
      double tmp;
      // cout<<mic<<endl;
       for(int k = 0; k < 53 ; k++) {
      tmp = energyNorm1->GetBinContent(k+1);
      if(tmp==0) continue;
      if(tmp<mic) {
	mic=tmp;
      }
       ...
       double half = ((1-mic)*.5)+ mic;
       

        TH1D *energy1 = (TH1D*)input3->Get(h3.c_str());
      if(energy1 == 0) continue;

      half = half * energy1->GetBinContent(31);
      
      if(caloSt == 13 && E < half) continue;

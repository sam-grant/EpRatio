...
void Plotter::Run() {
 ...
 TFile *input = TFile::Open("../makePlots/fitted_gain_by_xtal.root");
  ...
  while( NextallmuonsEvent() ) {
	...
	else if (xtal != lowLifeXtal[i]) {
	  brd = 2;
	}
	else { 
	  cout << "Error, no crystal match! xtal = " << xtal << endl; 
	  }

       int fidXtal = 0;
       if (FiducialXtal(trX, trY)) fidXtal = 1; //continue;
       else if(FiducialXtal(trX, trY)) fidXtal = 2;

       std::string h1 = "St"+std::to_string(caloSt)+"_fit_Ep_vs_xtal_0_"+std::to_string(xtal);
       std::string h2 = "St"+std::to_string(caloSt)+"_fit_Ep_vs_xtal_"+std::to_string(fidXtal)+"_"+std::to_string(xtal);
       ...
       // std::cout << caloSt << " " << xtal << " " << fidXtal << std::endl;
       ...
       //  Fill2D("St"+std::to_string(caloSt)+"_Ep_vs_xtal_0_"+std::to_string(xtal),xtal,Ep);

       Fill2D("St"+std::to_string(caloSt)+"_Ep_vs_t_early_0_"+std::to_string(brd),t,Ep1);

       //       Fill2D("St"+std::to_string(caloSt)+"_E_vs_xtal_0_"+std::to_string(xtal),xtal,E);
       
       // cout << " test 5 " << endl;
       // Fill2D("St"+std::to_string(caloSt)+"_Ep_vs_xtal_"+std::to_string(fidXtal)+"_"+std::to_string(xtal),xtal,Ep);

        Fill2D("St"+std::to_string(caloSt)+"_Ep_vs_t_early_"+std::to_string(fidXtal)+"_"+std::to_string(brd),t,Ep2);

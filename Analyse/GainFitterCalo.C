...
int main() {
  ...
  string inputFname = "RootFiles/PlotsTimeCalo.root";
  ...
  string outputFname = "RootFiles/FitsTimeCalo"+suffix+".root";
  TFile *output = new TFile(outputFname.c_str(), "recreate");
  
  // Define time limit
  const int maxTime = 4.2*cycles;

  cout<<"stn,N,chiSqrNDF,tau,dtau,alpha,dalpha"<<endl;
  // Station loop
  for (int stn(13); stn < 20; stn = stn + 6) {

      // Book histogram
      string h = "St"+to_string(stn)+"_Ep_vs_t";
      ...
      cout<<stn-1<<","<<N<<","<<chiSqrNDF<<","<<tau<<","<<tau_err<<","<<A<<","<<A_err<<endl;
        // Delete the canvas
      //      delete c1;

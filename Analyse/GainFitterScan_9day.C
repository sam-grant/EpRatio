...
void draw(vector<TH1D *> hist_, string fname, int stn, int par, int err, bool save) {
  ...
  for(int i(0); i<hist_.size(); i++) {
    ...
    else if(par == 0 && err == 1 && stn == 13) {
      y1 = 0.0;
      y2 = 0.04;
    }
    else if(par == 1 && err == 0 && stn == 13) {
      y1 = 2.0;
      y2 = 13.0;
    }
    else if(par == 1 && err == 1 && stn == 13) {
      y1 = 0.0;
      y2 = 5.0;
    }
    else if(par == 0 && err == 0 && stn == 19) {
      y1 = 0.004;
      y2 = 0.020;
    }
    else if(par == 0 && err == 1 && stn == 19) {
      y1 = 0.001;
      y2 = 0.007;
    }
    else if(par == 1 && err == 0 && stn == 19) {
      y1 = 4.0;
      y2 = 13.0;
    }
    else if(par == 1 && err == 1 && stn == 19) {
      y1 = 1.0;
      y2 = 5.0;
    }
    else {
      cout<<" *** Error: Parameter, uncertainty, or station not found *** "<<endl;
    }
   ...
   /*  hist_.at(i)->SetTitleSize(0.1f,"y");  */
    ...
    hist_.at(i)->GetXaxis()->SetTitleOffset(1.1);
    hist_.at(i)->GetYaxis()->SetTitleOffset(1.25);
    hist_.at(i)->GetXaxis()->SetTitleSize(0.04);
    hist_.at(i)->GetYaxis()->SetTitleSize(0.04);
    hist_.at(i)->GetXaxis()->CenterTitle(true);
    hist_.at(i)->GetYaxis()->CenterTitle(true);
  ...
  c->Draw();
  leg->Draw();
  if(save) c->SaveAs(("Plots_9day/"+fname+".pdf").c_str());
...
int main() {
  ...
  string input_fname = "RootFiles/PlotsTimeXtal_9day.root";

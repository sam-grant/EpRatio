  ...
  void draw(TH1D *hist0, TH1D *hist1, TH1D *hist2, TFile *output, string name, string title) {
    ...
    //tps1->SetTextColor(kOrange+7);
    tps1->SetLineColor(kRed);
    //tps1->SetLineWidth(0);
    tps1->SetFillColorAlpha(kRed,0.025);
    ...
    tps2->SetFillColorAlpha(kBlue,0.025);
    ...
    hist1->SetFillColorAlpha(kRed, 0.05);
    hist2->SetFillColorAlpha(kBlue, 0.05);
  ...
  int main() {
    ...
    string inputFnameEp = "RootFiles/EpParameters_9day_Q.root";//RootFiles/EpParameters_Q.root";
    string inputFnameLaser = "RootFiles/LaserParameters_9day.root";

    cout<<"Reading... "<<inputFnameEp<<endl;
    cout<<"Reading... "<<inputFnameLaser<<endl;

    string outputFname = "RootFiles/Statistics_9day.root";//1DStatPlots"+suffix+".root";
    ...
    TH1D *dummy = new TH1D("dummy","dummy",16,-8,8);
    TH1D *pullHistAmp = new TH1D("#alpha","pullHistAmp",16,-8,8);//7,-0.5,3); 
    TH1D *pullHistTau = new TH1D("#tau_{r}","pullHistTau",16,-8,8);//9,-3.5,1);//20*factor,-5,5);
    ...
    draw(dummy,pullHistAmp,pullHistTau,output,"ShortFits_9day/PullOverlay1D2.pdf",";Pull [#sigma];Entries / 1#sigma");

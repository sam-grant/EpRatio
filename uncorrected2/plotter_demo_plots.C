// Plotter to make enegy E/p plots
// Select positrons
// Select high flux region energy, defined to leave no bin left out. 1.2 - 2.4 GeV!
// 3% energy resolution, leading to 200 MeV bin width -> 20 bins
// Plot all tracks and fiducial region
...
void Plotter::InitHistos() {
  ...
  plot2D("E_vs_p_0",500,0,4000,500,0,4000,"Track Momentum [MeV]","Cluster Energy [MeV]");
  plot2D("Ep_vs_t_0",50,0,4200*50,500,0,4,"In Fill Time [ns]","E/p");

  plot2D("E_vs_p_1",500,0,4000,500,0,4000,"Track Momentum [MeV]","Cluster Energy [MeV]");
  plot2D("Ep_vs_t_1",50,0,4200*50,500,0,4,"In Fill Time [ns]","E/p");

  plot2D("E_vs_p_2",500,0,4000,500,0,4000,"Track Momentum [MeV]","Cluster Energy [MeV]");
  plot2D("Ep_vs_t_2",50,0,4200*50,500,0,4,"In Fill Time [ns]","E/p");
...
void Plotter::Run() {
  ...
  while( NextallmuonsEvent() ) {
	...
	if(dt<-9 || dt>1) continue;        
      ...
      if (200 < E) region[1]=true;
      //      if(E/p < 0.5) region[1]=true;
      ...
      Fill2D("E_vs_p_0",p,E);
      Fill2D("Ep_vs_t_0",t,Ep);
      if(region[0]){
	Fill2D("E_vs_p_1",p,E);
	Fill2D("Ep_vs_t_1",t,Ep);
      }
      if(region[2]) {
	Fill2D("E_vs_p_2",p,E);
	Fill2D("Ep_vs_t_2",t,Ep);
      }

...
void Plotter::Run() {
 ...
 TFile *input2 = TFile::Open("../makePlots2/fits_xtal_energy_normalised_cut.root");      
 ...
 while( NextallmuonsEvent() ) {
	...
	if (xtal == shortLifeXtal[j]) {
      ...
      ////////////////////////
      // Energy fraction cut
      std::string h3 = "St"+std::to_string(caloSt)+"_fit_E_vs_xtal_0";
      std::string h4 = "St"+std::to_string(caloSt)+"_fit_E_vs_xtal_1";
      TH1D *cut1 = (TH1D*)input2->Get(h3.c_str());
      TH1D *cut2 = (TH1D*)input2->Get(h4.c_str());
      double cutValue1 = cut1->GetBinContent(xtal+1);
      double cutValue2 = cut2->GetBinContent(xtal+1);
	...
	// }
	if (FiducialXtal(trX, trY) && cutValue2 != 0) {

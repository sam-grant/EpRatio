  ...
  for (int stn = 13; stn < 20 ; stn = stn + 6) {
    for (int fidXtal = 0 ; fidXtal < 3 ; fidXtal++) { 
      for (int board = 1 ; board < 3 ; board++) {
        plot2D("St"+std::to_string(stn)+"_Ep_vs_t_early_"+std::to_string(fidXtal)+"_"+std::to_string(board),50,0,4200*50,200,ymin,ymax,"t [ns]", "E/p");
      }
    }
  }
}
...
void Plotter::Run() {
      ...
      TFile *input = TFile::Open("../makePlots/xtal.root");
  ...
  while( NextallmuonsEvent() ) {
	...
	if(dt<-9 || dt>1) continue;        
      ...
      int board = 2;
      for (int i = 0 ; i < 22 ; i++ ) {
	if (xTal == lowLifeXtal[i]) {
	  board = 1;
      	}
      }
      // cout << xTal << " " << board << endl;
 
      // if (t < 60000) continue;
      // Terrible and shameful method of scaling gain per crystal
      std::string name_0 = "St"+std::to_string(caloSt)+"_fit_Ep_vs_xtal__0";
      std::string name_fid = "St"+std::to_string(caloSt)+"_fit_Ep_vs_xtal__"+std::to_string(fidXtal);
      TH1D *scale_0 = (TH1D*)input->Get(name_0.c_str());
      TH1D *scale_fid = (TH1D*)input->Get(name_fid.c_str());

      double scaleFactor_0 = scale_0->GetBinContent(xTal); 
      double scaleFactor_fid = scale_fid->GetBinContent(xTal); 
      
      double Ep_0 = Ep / scaleFactor_0;
      double Ep_fid = Ep / scaleFactor_fid;
      //   std::cout<<xTal<<" "<<scaleFactor_0<<std::endl;

       Fill2D("St"+std::to_string(caloSt)+"_Ep_vs_t_early_0_"+std::to_string(board),t,Ep_0);
       Fill2D("St"+std::to_string(caloSt)+"_Ep_vs_t_early_"+std::to_string(fidXtal)+"_"+std::to_string(board),t,Ep_fid);

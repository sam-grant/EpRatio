...
void Plotter::InitHistos() {
  for (int stn = 13; stn < 20 ; stn = stn + 6) {
    for (int fidXtal = 0; fidXtal < 2; fidXtal++) {
      plot2D("St"+std::to_string(stn)+"_E_vs_xtal_"+std::to_string(fidXtal),54,-0.5,53.5,20,0,1.5,"Crystal Number", "Energy Normalised to Xtal 30");
...
void Plotter::Run() {
  ...
  TFile *input1 = TFile::Open("../makePlots2/fits_xtal_energy.root");
  ...
  while( NextallmuonsEvent() ) {
	...
	if(dt<-9 || dt>1) continue;        
      ...
      //////////
      //Normalise
      std::string h1 = "St"+std::to_string(caloSt)+"_fit_E_vs_xtal_0";
      std::string h2 = "St"+std::to_string(caloSt)+"_fit_E_vs_xtal_1";

      TH1D *E_vs_xtal_0 = (TH1D*)input1->Get(h1.c_str());
      TH1D *E_vs_xtal_1 = (TH1D*)input1->Get(h2.c_str());

      double sf1 = E_vs_xtal_0->GetBinContent(31);
      double sf2 = E_vs_xtal_1->GetBinContent(31);
      cout <<  sf1 << endl;
      double E1 = E / (sf1*1000);
      double E2 = E / (sf2*1000);
      
      //  cout<<E1<< " " <<E2<<endl;
      
      ///////////////////////////////////////////////////////////////////////////
      Fill2D("St"+std::to_string(caloSt)+"_E_vs_xtal_0",xtal,E1);
      Fill2D("St"+std::to_string(caloSt)+"_xy_calo_0",trX,trY);
      if (FiducialXtal(trX, trY)) { 
	Fill2D("St"+std::to_string(caloSt)+"_E_vs_xtal_1",xtal,E2);

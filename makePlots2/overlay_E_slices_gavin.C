...
int main() {
  ...
  TFile *mipFile = TFile::Open("unnormalised_MIPs.root");
 ...
 mu_u->SetTitle("Lost #mu^{+}");
    ...
    lo_u->SetTitle(("Calo "+to_string(stn)+" | Gain Uncorrected Ratios;Crystal Number; Arb. Units").c_str());
      ...
      mu_c->GetYaxis()->SetRangeUser(155,185);
      //  c5->cd(1)->BuildLegend(0.69,0.79,0.89,0.89);
      //  c5->cd(1)
      mu_c->Draw();
      mips->Draw("same");
     c5->cd(1)->BuildLegend(0.69,0.79,0.89,0.89);
      ...
      //gPad->SetGrid();
       ...
       mu_c->SetTitle(("Calo "+to_string(stn)+" | MIPs, Gain Corrected;Crystal Number;Cluster Energy [MeV]").c_str());
       
       c5->cd(2);
       TH1D *ratio = (TH1D*)mips->Clone("ratio");
       ratio->Divide(mu_c);
       ratio->SetTitle(("Calo "+to_string(stn)+" | Ratio: Nandita/Sam;Crystal Number; Ratio").c_str());
       ratio->SetLineColor(kBlack);
      ratio->SetLineWidth(2);
       gPad->SetGrid();
       ratio->GetYaxis()->SetRangeUser(0.9,1.05);
       ratio->Draw();

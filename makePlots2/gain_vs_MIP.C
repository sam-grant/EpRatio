...
int main() {
  ...
  string line_g;
  string line_m;
  ifstream gainFile("gain_means_new_normalised.csv");
  ifstream mipFile("../makePlots/mip_peaks_norm_by_xtal22.csv");
 ...
 //  for (int i = 55 ; i < 109 ; i++){
  ...
  mip_h->SetMarkerColor(kBlack);
  mip_h->SetLineColor(kBlack);
  gain_h->SetMarkerColor(kRed);
  gain_h->SetLineColor(kRed);
  mip_h->GetYaxis()->SetRangeUser(0.7,1.1);//xmin,xmax);
  ...
  c1->BuildLegend(0.69,0.79,0.89,0.89);
   mip_h->SetTitle("St 13 | MIP peaks & gaussian mean E/p, normalised to xtal 22;Crystal Number;Normlised Units");
  c1->SaveAs("st13_mip_Ep_per_xtal.png");
   delete c1;
  delete gain_h;
  delete mip_h;
